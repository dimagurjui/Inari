...
class Volpi implements ArrayAccess
{
    ...
    private function insertStringLimits()
    {
        $pattern = '/{{\s*str_limit\(\s*(.+?)\s*,\s*([0-9]+)\s*\)\s*}}/';
        $replacement = '<?php echo substr($$1, 0, $2).\'...\'; ?>';
        $this->content = preg_replace($pattern, $replacement, $this->content);
    }

    private function insertAuthLogic()
    {
        $patterns = array(
            '/{{\s*loged\s*}}/' =>
                '<?php if( User::logged() ) echo \'',
    ...
    private function compile($template)
    {
        ...
        $this->insertSections();
        $this->insertAuthLogic();
        $this->insertStringLimits();
        $this->insertVars();
        $this->insertCycles();
    }
